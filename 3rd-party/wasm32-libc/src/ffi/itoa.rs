...
pub unsafe extern "C" fn utoa(mut u: u64, s: *mut CChar, s_len: usize, radix: u8) -> i32 {
    ...
    for slot in buffer_slice.iter_mut() {
        ...
        if digit <= 9 { *slot = digit + b'0'; }
        else { *slot = digit - 10 + b'a'; }
        index += 1;
        u /= radix as u64;
        if u == 0 { break; }
    }
    if u != 0 { return -1; }
    if index < buffer_slice.len() { buffer_slice[index] = b'\0'; }
...
mod test {
    ...
    fn hex() {
        let mut buf = [b'\0'; 32];
        assert_eq!(unsafe { itoa(0xDEADBEEF, buf.as_mut_ptr(), buf.len(), 16) }, 8);
        assert_eq!(unsafe { strcmp(buf.as_ptr(), b"deadbeef\0" as *const u8) }, 0);
    }
